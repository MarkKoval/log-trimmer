...
from core import DataFlashExporter, DataFlashParser, LogInfo, Segment, normalize_segments, remove_segments
...
class MainWindow(QMainWindow):
    ...
    def open_log(self, path: Path) -> None:
        try:
            parser = DataFlashParser(path)
            self.log_info = parser.summarize()
            self.series = parser.collect_series()
        except Exception as exc:  # noqa: BLE001
            logger.exception("Failed to open log: %s", exc)
            QMessageBox.critical(self, "Open error", f"Failed to open log: {exc}")
            return
    ...
    def _export(self) -> None:
        ...
        try:
            exporter.export(Path(dest), self.remove_segments, progress_cb=on_progress)
        except Exception as exc:  # noqa: BLE001
